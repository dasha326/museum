  ...
  <div class="player">
    <div class="player-controls">
      <div class="player-controls__item">
        <button @:click="setTrek('prev')" title="Предыдущая" type="button" v-show="isPrevBtnVisible">
          prev
        </button>
      </div>
      <div class="player-controls__item">
        <button @:click="setTrek('next')" title="Следующая" type="button" v-show="isNextBtnVisible">
          next
        </button>
      </div>
      <div class="player-controls__item">
        <button @:click="playing = !playing" :title="playing ? 'Пауза': 'Воспроизвести'" type="button">
    ...
    <audio ref="audiofile" :src="activeAudio" preload="auto" controls></audio>
  </div>
...
export default {
  ...
  watch: {
    ...
    audioIndex: function(value) {
      this.activeAudioIndex = value;
      this.stop();
    },
    activeAudioIndex(){
      this.stop();
  ...
  methods: {
    fileLink(file){
      return `../assets/${this.personFolder}/${file}`;
    },
    update() {
    ...
    setTrek(direction){
      ...
      let index = this.activeAudioIndex;
      if (direction === "prev") {
        if (index > 0) index--;
      }
      if (direction === "next") {
      ...
      index === playList.length-1 ? this.isNextBtnVisible === false : this.isNextBtnVisible === true;
      console.log(index === 0);
      console.log(this.isNextBtnVisible, this.isPrevBtnVisible);
      this.activeAudioIndex = index;
    },
  },
  computed: {
    activeAudio(){
      return this.fileLink(this.playList[this.activeAudioIndex].file)
    },
  },
  //   download() {
  //     this.stop();
  ...
  created() {
    this.activeAudioIndex = this.audioIndex;
  }
}
</script>

<!--https://github.com/tutsplus/build-a-music-player-with-vuejs-and-vuetify/blob/master/src/components/PlayerControlsBars.vue-->
<style scoped>
</style>
